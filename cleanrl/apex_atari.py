...
def act(args, epsilon, experiment_name, i, q_network, target_network, lock, rollouts_queue, stats_queue, global_step, device):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--batch-size", type=int, default=512, help="the batch size of sample from the reply memory")
    parser.add_argument("--start-e", type=float, default=0.4, help="the starting epsilon for exploration")
    ...
    epsilon_values = np.random.uniform(low=0.00001, high=0.01, size=args.num_actors)

    for i in range(args.num_actors):
        epsilon = epsilon_values[i]
        actor = ctx.Process(
            target=act,
            args=(args, epsilon, experiment_name, i, q_network, target_network, lock, rollouts_queue, stats_queue, global_step, "cpu"),
        )
        actor.start()
